...
配置在**这里**读一次、之后**每条单元再重读一次**
----------------------------------------------
装配期强校验（缺平台表 / 缺账号 ⇒ 起不来，而不是发到一半才炸）。

但**跑起来之后配置还会被改**：发布面板的「账号配置」就在改 ``config/publish.yaml``。
钉着启动那一份的后果是"面板上明明配好了、什么都没发出去"，而且**发布面板上连一条
记录都不会出现**（守卫跑在建 ``publications`` 那一行之前）—— 真机坑 2026-09-23。
所以 ``build_publish_handler`` 会注入一个 loader，让每条单元认领前重读
（见 ``PublishPlatformHandler._refresh_config``）。代价是每条多读一次配置，
而一条发布单元本身要开浏览器跑八步 —— 可以忽略。
