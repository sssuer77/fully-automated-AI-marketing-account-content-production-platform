...
> 依据：`docs/spec/`（**v3.2 终版 · 需求已冻结**）｜ 生成日期：2026-09-13 ｜ 最近更新：2026-09-15（**T1.1–T1.12 全部 ✅** + **T4 已完成 11/13**：T4.1 前端脚手架 ✅ · T4.2 总览台 ✅ · T4.3 选题面板 ✅ · T4.4 稿件面板 + 确认闸 ✅ · T4.7 合成配置面板 ✅ · **T4.8 素材库 ✅** · T4.9 实时日志 ✅ · T4.10 四池调度控制台 ✅ · T4.11 无人值守 ✅ · T4.12 观测/备份/交付 ✅ · T4.13 人物库面板 ✅）
...
- [ ] 归一化：数字 / 百分比 / 日期 / 英文缩写 / 多音字 / emoji
- [ ] **幂等性**：`normalize(normalize(x)) == normalize(x)`
- [ ] 标点 → 停顿映射
- [ ] 切分：单句 **4–28 字**，不破坏语义边界（不在数字/英文单词中间切）
- [ ] glossary 热更新（不改代码即可纠正读音）
- [ ] **不引入 `pynini`/`WeTextProcessing`**（Windows 装不上）
- ✅ `pytest tests/unit/tts/test_normalize.py -q`（**≥40 条黄金用例**）；`pytest tests/unit/tts/test_segmenter.py -q`；幂等性属性测试
- ⚠️ **R5** `pynini` 不可用 ⇒ 自研 `text_normalize`，**不 import `tn`**
...
> 注 1：`T1.2` 含 **T1.2+ persona 可编辑改造**（人物库 / 热重载 / 一键切换 / 自动备份）。E7 现有 2 套可跑人物（`persona_default` 熊大熊二 · `solo_commentary` 快嘴单人），**仍待你确认口吻 / 受众 / 禁区**。
>
