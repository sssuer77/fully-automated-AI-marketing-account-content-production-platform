...
**参考音能有多短（2026-09-22 · 裁定 369）**

上游**只挡长的**：`cosyvoice/cli/frontend.py` 里那一句
`assert speech.shape[1] / 16000 <= 30`（"超过 30 秒的提取不了 speech token"）——
全文没有任何"太短"的判据。真机也验过：

| 项 | 读数 |
| --- | --- |
| 参考音 | `data/voice_src/sunxiaochuan/ref_01.wav`，**2.978 秒** @ 44.1 kHz 立体声 |
| 参考文本 | `ref.txt` 第 1 行 |
| 目标文本 | 「这是三秒参考音的试听，看看像不像。」 |
| 结果 | `ok=true` · 出 **4.48 秒**音频 · `synth_ms=5093` · **RTF 1.137** · `ref_wav=ref_01.wav` |

所以素材库那条「单段 10 秒」的下限**不是引擎的要求**，已降到 2 秒（30 秒上限保留 ——
那条是真的）。另外注意 `VoiceRegistry.resolve` 只取 `wavs[0]` + `ref.txt` 的第一行：
**第 2、3 段在合成时不参与**，"2–3 段"是入库留痕的约定，不是克隆的输入。
