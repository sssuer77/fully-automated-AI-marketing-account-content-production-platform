...
| **T3.4** 🔶 | **单遍合成执行器**：argv 数组（**不用 shell**）、`-progress pipe:1` 进度解析、超时/取消（杀进程树）、`.partial` 原子改名、stderr 截断留痕、`composite_hash` 整片缓存 | T3.3 | ✅ **已落地**：argv 数组（`build_composite_argv()` → `run_command(argv)`）、`.partial` + 原子改名（`composite.py:358`）、stderr 尾部截断（`CommandResult.tail()`）+ 错误码映射、`composite_hash` 计算（写入 `manifest.json` / `quality_json.plan_hash`）—— `pytest tests/unit/render/test_degrade.py -q` ⇒ **8 passed**。❌ **一期未做**（2026-09-17 核对）：`-progress pipe:1` 进度解析与 2Hz 限流（argv 用 `-nostats`，面板进度是粗粒度的）、超时**杀进程树**（`core/media.py:105` 的 `run_command()` 用 `subprocess.run(timeout=)`，超时 ⇒ `returncode=-2`，**没有** `taskkill /T /F`）、`composite_hash` **缓存复用**（hash 算了但**没有**「同哈希不调 ffmpeg」的分支）、`BELOW_NORMAL_PRIORITY_CLASS`（`media.py:126` 用 `CREATE_NO_WINDOW`）；`pytest tests/integration/test_composite_runner.py -q`（**文件不存在**） | 崩溃留「假完成」⇒ `.partial` + 原子改名（陷阱 #9）；僵尸 ffmpeg ⇒ **杀进程树一期未做**；**缓存复用旧产物** ⇒ 哈希含 canonical plan + 输入 sha256（陷阱 #8，**哈希已算、缓存未接**）；NVENC 失败 ⇒ 回退 `libx264` |
