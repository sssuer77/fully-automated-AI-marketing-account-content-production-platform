...
| **二线** | 小红书 `xiaohongshu`、B站 `bilibili`、西瓜视频 `xigua`、微博 `weibo` | ✅ 实现已接线 · ⚠️ 选择器未校准 | 与一线走**同一个** `PlaywrightPublisher`，四个子类只声明平台代号（T5.14）；`selectors/<code>.yaml` 里那份 CSS **没在真机上校准过** ⇒ 出厂 `enabled: false`，面板上带 ⚠️ |

> Q9 默认：一期只做一线；二线保留接口与 profile，二期按实际投放需求逐个补齐。
>
> **2026-09-23 更新（T5.14）**：二线那四个平台已经从"空实现"改成**真实现**，但那份 CSS
> **没在真机上校准过** ⇒ `enabled` 仍是 `false`。**能不能真发**由 `selectors/<code>.yaml`
> 的 `calibrated` / `calibrated_at` 回答（现在只有抖音是 `true`），面板上那一列直接照着它显示。
> **"没做"与"做了但没校准"必须分得开**：空实现时代这两件事看起来一模一样，而它们该做的事
> 正好相反（一个别等，一个去校准）。
...
>
> 校准的入口是 `uv run studio publish calibrate --platform <code>`（**只读**探针：开一个可见
> 窗口、把 pack 里每条选择器在真页面上问一遍，不点发布、不填输入框）。用法与"校完写回哪几个
> 字段"见 `docs/runbook/publish_selector.md` §2.2。
...
# ⚠️ 上面四个的 `enabled: false` 从 T5.14 起**不是**"一期没做"，而是"实现做了、选择器还没
#    在真机上校准过"。逐个跑 `studio publish calibrate`、写回 `calibrated: true` 之后再翻成
#    `true` —— 顺序反过来的话，面板上会出现一个"能投、但一定发不出去"的平台。
...
| 发布单元 | `jobs.unit_ref = platform:account_id`（T5.8 落地）；**老格式（只有平台代号）仍然认** |
| 分发口径 | 投递不带 `account_ids` ⇒ 发到该平台**全部**启用账号（矩阵默认意图）；点名 ⇒ 只发点名的那些 |
| 面板操作 | 发布面板「投递」区块：勾平台 = 该平台**全投**；多账号平台下面可以直接取消其中几个号 |
| 面板配置（T6.4） | 发布面板「账号配置」区块：**加号 / 改号 / 停用·启用 / 删号**，按行写回 `config/publish.yaml`（段外一个字节不碰）。**删号不删 `data/browser_profile/<id>/`**（那是凭据，§02.5）；新号仍要人工扫码一次（R13：不自动登录） |
| 面板扫码登录（T6.4） | 同一区块里点「**扫码登录**」⇒ 在**跑着 studio 服务的那台电脑**上弹出一个浏览器窗口（平台自己的登录页）⇒ 用手机上对应的 App（抖音/快手/视频号）里的「扫一扫」扫那个码 ⇒ 窗口自己关掉。**没有密码**：本系统不存密码、不自动登录、不绕过验证码（R13）。另有「检测登录态」＝无头探一眼（几秒钟、不发任何东西） |

> **设计意图**：现在单账号跑通闭环；将来做矩阵时，**只改配置不改代码**。

**T5.8 落地记录（2026-09-19）** —— 上表六条里，**结构那几条**（`account_id` 列、独立 `profile_dir`、
按账号限频、幂等键含账号）从 T5.3 起就在了；本轮补的是**"真的能把第二个账号发出去"**：

| 缺口 | 落地 |
| --- | --- |
| 发布单元只写平台代号 ⇒ 第二个账号投不出来 | `jobs.unit_ref = platform:account_id`；老作业行仍认（`parse_unit_ref`） |
| 投递一次只能点一个账号 | `enqueue_publications(account_ids=[...])` 按 **平台 × 账号** 展开；面板可勾账号 |
| 定时计划一个平台只发第一个账号 | `SchedulerService._targets()` ⇒ 一个平台下**每个启用账号**都是一个目标 |
| 报告看不到账号维度 | `reports.data_json.by_account` + 报告正文「账号归因」一节 |

> **`account_ids` 是全局名单、按平台取交集**：面板上的账号勾选框是**分平台**画的，而请求体里
> 只有一份名单。取交集是唯一能表达"勾了 douyin 的 acc_b、同时也要发 kuaishou"的读法；
> 严格版（出现别的平台的账号就报错）会把另一平台的账号读成"这个平台的误配"，
> 于是面板上勾了两个平台、点一次投递，其中一个被整条跳过 —— 而用户什么都没做错。
> **交集为空才算错**（这个平台上点名的账号一个都不存在），文案里带上这个平台有哪些账号。
...
> **但"只差不可见字符"不算不一致**（裁定 389 · 真机 2026-09-23）：平台的富文本编辑器会在
> 文案末尾塞一个**零宽空格**（U+200B）当哨兵 —— 逐字比对看得见它，人看不见。于是比对
> **两遍**：先按原样比；不一致时把"纯排版控制符"（软连字符 / 零宽空格 / LRM / RLM /
> 双向嵌入与隔离 / 词连接符 / BOM）两边都去掉再比 ⇒ 一样就判 `invisible_only` 并**放行**，
> 同时把差异字符写进日志。`TOLERATED_READBACK_REASONS` **只此一项**，**刻意不收**
> U+200C / U+200D —— 它们参与 emoji 与印度语系的字形组合，收进来等于顺手放行"emoji 被吞"，
> 而那正是回读要抓的东西（陷阱 #224）。

> **第 ⑦ 步有两个判据，而且"平台要人"必须单独成一档**（裁定 391 · 392 · 真机 2026-09-23）：
> ① **元素判据** `success_marker` / `reject_marker` —— 一律用 `:text-is()`（**全等**）：
> `:text()` 是**子串**，而页面上那句「视频发布成功**后**，价格将无法更改」里就有"发布成功"
> 四个字 ⇒ 会在**发布之前**就判成成功（陷阱 #229）；驳回判据**先于**成功判据判。
> ② **地址判据** `markers.success_url_contains` —— 抖音发布成功后**不发**"发布成功"，而是把
> 页面跳到内容管理列表；只认元素的话，一条**已经发出去**的内容会在 600s 之后被记成失败
> （陷阱 #228）。
> ③ **`verify_marker`**（「接收短信验证码」/ 人脸验证）⇒ 报 `PUBLISH_FAILED` 并**转
> `manual_required`**（**不是** `failed`）：**R13 不自动登录、也不替人过验证** —— 它是
> "自动这条路走完了、接下来等人"，所以错误信息要写成一句能照做的话，并打开 `can_mark_done`
> （陷阱 #230）。

> **④ 「人工过验证」：同一条八步，只差"窗口前面有没有人"**（裁定 393 · 真机 2026-09-23）：
> 平台弹验证框时，人在场的那条路**不分叉** —— 走的还是这个 `PlaywrightPublisher`、这份
> 选择器、这八步，唯一的差别是 `PublisherContext` 上两个开关
> （`headless=False` + `await_manual_verify=True`）。没人时"弹框"是收场（③）；
> 有人时它是"轮到你了"：第 ⑦ 步**继续等**，等到的仍是原来那三条判据（驳回 / 成功元素 /
> 地址跳转）—— **"框消失了"本身不算成功**（人可能只是点了取消）。
> 等**人**的上限是 `MANUAL_VERIFY_WAIT_SEC`（900s，比等机器的 600s 长一个量级），
> 入口是面板上的「人工过验证」（`POST /api/v1/publish/{id}/assist`，
> `services/publish_assist_service.py`）：它**不碰作业队列**，直接对着那条记录发，
> 结论照旧落 `published` / `manual_required`。系统仍然**不点**「获取验证码」、
> **不读**短信、**不填**码、不存任何凭据。
...
**落地（T5.7 · 2026-09-18）**：九条规则**全部实现**（`services/report_service.py` + 9 个 REST 端点 +
面板第 ⑫ 块）。三处按施工裁定收窄/明确：

- **窗口**：不是两个自由参数，而是「右端 = 触发日的**前一天**，左端由 `lookback_days` 倒推」（§6.7.5 的
  "周一生成上周" 正是这个规则在 `weekly + lookback=7` 下的样子）。今天还没过完，算进去只会让每份报告都偏低。
- **图表**：一期**不出 PNG**（`format` 只支持 `md` / `csv`）。`data_json` 已经带齐所有维度的数字，
  面板直接渲染成表格；画一张"看着像真的"图，比不画更糟（裁定 299 的同一条理由）。
- **"采纳/忽略"**：一期只有**采纳**（`POST .../insights/{idx}/apply`）。忽略**不落库** —— 没有落点的
  "已忽略"标记会在下一份报告里消失，而人会以为自己标记过。要改主意就别点，下一份报告照常重算。
...
| **账号配置**（T6.4） | 账号清单（**含停用的**）+ 每行一句「现在什么状态、下一步做什么」；`profile_dir` 与运行期真正用的那个目录不一致时**当场说出来** | 加号 / 改号 / 停用·启用 / 删号（写回 `config/publish.yaml`，**不删登录态目录**）；**检测登录态**（无头，几秒）；**扫码登录**（弹出可见窗口等人扫，最长 3 分钟） |
...
[x] T5.8 多账号（D1）：结构支持多账号、默认单账号；加账号零迁移；限频按账号独立计数（**2026-09-19 落地**）
