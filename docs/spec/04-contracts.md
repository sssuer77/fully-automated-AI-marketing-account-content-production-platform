...
class PublishMetrics(BaseModel):
    ...
    collected_at: str
    # ★增补（T5.9）：完播率，**0–1 的比值**（0.423 = 42.3%）。
    #   四个计数回答"有多少人看"，它回答"看完的有多少" —— 播放量高、完播率低
    #   说明标题骗进来了、内容没接住，这两条给选题的指令正好相反。
    #   存比值而不是百分数：百分数一旦在某个环节被当成比值（或反过来），误差是
    #   100 倍，而 42 与 0.42 都是"看着正常"的数。None = 我们不知道（**不是 0**）。
...
``PUBLISH_DISABLED`` **进死信**（★更正 2026-09-18：原文写的是"转人工"，与 ① 的开关守卫
那条自相矛盾 —— 守卫抛在**建 ``publications`` 那一行之前**，所以既没有待人工记录、
发布面板上也什么都不出现；真实落点是死信，在「四池调度」里能看到）。投递期直接拒绝的话，
面板上连作业都没有 —— 而"点了没反应"比"有一条能查的作业"难查得多。

> 投递面板因此把这句话**写在选项旁边**（T5.10）：开关关着时，真平台那一条会明说
> "投了会直接死信、发布面板上不会出现记录"。出厂配置下不说这句，操作员按一次投递
> 会得到"什么都没发生"，而那是 R14 的正常行为 —— 最容易被读成"按钮坏了"的一种。
...
### 4.6.9 成片库契约（T5.11 · 已落地）

发布这条路此前**只能手敲任务号**：面板上那一格要人把一个 26 位的 ULID 抄进去，而人手里有的是
**一支片子**（他刚在渲染面板看着它出出来），不是一串 id。这一节补的就是那个连接 ——
盘上有什么 ⇒ 它是哪条任务的 ⇒ 它发到哪儿了。

**① 两个端点**

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | ``/api/v1/library`` | 盘上已出的片子，**一条任务一行**（新的在前）。``limit`` 1–500（缺省 200） |
| POST | ``/api/v1/library/publish`` | **批量**投递：``task_ids[]``（1–50）+ ``platforms[]`` + ``dry_run``，逐条回结论 |

``GET`` 的 ``items[].publications`` 直接嵌 ``PublicationView`` —— ``can_retry`` / ``can_cancel``
那几个布尔**只有一份**（``publication_view()``，发布面板用的是同一个）。在成片库里另起一个
"精简版"的代价是：发布面板上写着"可以重试"，而成片库里那一行说"不行"，两边各自成立，
而**没有任何地方会报错**。

**② 一行 = 一条任务，不是一个文件**

发布池认的是**任务号**：它自己拿 ``resolve_final_video(task_id)`` 去找片子（``manifest.json``
优先、目录按名字兜底），**不认调用方指的那个文件**。所以面板按文件列的话（一条任务重出过
三版就是三行），勾第二行与勾第三行的结果**一模一样** —— 那是一句谎话，而且是最难查的那种
（"我明明选了新那一版"）。按任务列，把"盘上有几版"如实写在 ``versions`` 里，它只是**信息**。

文件名 ⇒ 任务号那一步是 ``library_service.parse_task_id``：``{yyyymmdd-HHMMSS}_{task_id}_final[_720p].mp4``。
``.+`` 是**贪婪**的（不是 ``.+?``）—— 任务号本身可以含下划线（``ui-2026…``、用户自己起的名），
非贪婪会在任务号里遇到第一个 ``_final`` 就断，切出一个**不存在的任务号**，而它在面板上
看起来完全正常。认不出来的一律**不进库**（``data/output/videos/`` 是人也会放东西的地方），
不抛异常。

**③ 为什么批量投递是**一个**端点**

让面板循环调 N 次 ``POST /publish/tasks/{id}/enqueue`` 的代价有三条，都不是理论上的：
① 勾 20 条就是 20 个请求，中间断一次（刷新、切屏）会留下一半投了一半没投的状态，而面板上
没有任何地方记得"刚才投到第几条"；② 每一条的失败要在前端各拼一次文案，于是"douyin 平台未启用"
这句话会出现在**前端**；③ 上限只能在前端判，而前端判据与后端分叉时，用户看到的是
"按钮能点、点了报错"。所以：**一次请求，一次判据，逐条回结论**。

请求体本身不合法（空清单 / 超过 ``BATCH_MAX``）⇒ 422 ``VALIDATION_FAILED``：那是**调用方**的错，
不该混进 ``items`` 里冒充"这一条没投出去"。同一个任务号写两遍会先去重。

**④ 投递期就挡掉两种「发不出去」**

``publish_many`` **逐条**判，两种不进池：

- 任务不存在 ⇒ ``missing=true``。**先判任务、再判成片**：反过来的话，"任务号打错了"会报成
  "盘上没有成片" —— 两句都是真话，但指向的下一步动作完全不同（改任务号 vs 先去出片）。
- 盘上没有成片 ⇒ ``skipped=["盘上没有成片"]``。投出去的话，发布池会在认领之后报
  ``RENDER_FAILED``、那条作业进死信、``publications`` 里连一行都没有 —— 用户看到的是
  "投了，然后没了"。在投递这一侧说清楚，比在死信里说清楚要早一步（人还在面板上的时候）。

``duplicates``（幂等命中）与其余 ``skipped`` **分开**：前者的处置动作是"什么都不用做"，
合成一句"跳过 2 条"之后，操作员会去改一个本来就没问题的配置（陷阱 182 的同族）。

**⑤ 投递**不看** ``publish.enabled``**（与 4.6.7 ⑥ 同一条）

开关关着时投递照样成功，作业在 worker 那一侧带 ``PUBLISH_DISABLED`` 进死信。面板因此把
这句话**写在真平台选项旁边**（``publish_enabled`` 来自服务端）：出厂配置下不说这句，
操作员按一次「批量发布」会得到"什么都没发生" —— 最容易被读成"按钮坏了"的一种。

**⑥ 为什么成片库**不轮询**

这一屏看的是"盘上有什么"—— 它只在人出片 / 删片时变。空闲时每两秒拉一次全库，拉回来的是
同一份东西。所以靠"进来拉一次 + 一颗刷新"，代价说清楚：**别人在命令行刚出的那一条不会自己
冒出来**。
